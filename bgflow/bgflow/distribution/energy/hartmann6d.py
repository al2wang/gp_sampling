...
__all__ = ["OpenMMBridge", "Hartmann6D"]
...
class Hartmann6D(nn.Module):
    """Hartmann-6D analytical energy replacement for Hartmann6D.
    Provides energies and gradients in the same interface as Hartmann6D.
    """
    def __init__(self, dim=6, two_event_dims=False):
        super().__init__()
        self.dim = dim
        self.two_event_dims = two_event_dims
    ...
    def forward(self, x, compute_gradients=True):
        """Compute energy and (optionally) forces like Hartmann6D.
        ...
        compute_gradients : bool
            Whether to return forces (negative gradients)

        Returns
        -------
        energy : torch.Tensor of shape (batch_size, 1)
        forces : torch.Tensor of shape (batch_size, dim), or None
        """
        x = x.view(x.size(0), self.dim)
        x = torch.clamp(x, 0.0, 1.0).to(x.device)
        x.requires_grad_(compute_gradients)

        # Compute energy
        outer = torch.zeros(x.size(0), device=x.device)
        for i in range(4):
            inner = torch.sum(self.A[i] * (x - self.P[i])**2, dim=1)
            outer += self.alpha[i] * torch.exp(-inner)
        energy = -outer.unsqueeze(1)

        if not compute_gradients:
            return energy, None

        # Compute gradients (forces = -∇E)
